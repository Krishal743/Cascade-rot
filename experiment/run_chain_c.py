"""Experiment runner for Chain C (calendar chain - 5 steps)."""
...
from tools.schemas import CHAIN_C
from perturbations.engine import PerturbationEngine
from perturbations.dataset import CHAIN_C_QUERIES, PERTURBATION_TYPES, SEVERITY_LEVELS
...
def generate_test_dataset(num_queries: int = 5) -> list:
    """Generate test dataset - 55 instances (5 queries × 11 perturbation combos)."""
    engine = PerturbationEngine(severity="moderate")
    dataset = []
    
    selected_queries = CHAIN_C_QUERIES[:num_queries]
...
def run_chain_c_experiment():
    print("=" * 70)
    print("CHAIN C EXPERIMENT (Calendar - 5 steps)")
    print("=" * 70)
    
    model, tokenizer = load_model()
    runner = ChainRunner(model, tokenizer, CHAIN_C)
    ...
    for i, item in enumerate(dataset):
        ...
        trace = runner.run(query, chain_name="chain_c")
    ...
    with open("results/chain_c_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to results/chain_c_results.json")
